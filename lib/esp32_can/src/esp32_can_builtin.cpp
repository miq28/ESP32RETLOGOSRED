...
void ESP32CAN::enable()
{
    ...
    if (!rxRing)
    {
        rxRing = (RxRing *)heap_caps_malloc(sizeof(RxRing), MALLOC_CAP_INTERNAL);
        if (!rxRing)
        {
            Serial.println("RX ring allocation FAILED");
            return;
        }
        rxRing->head = 0;
        rxRing->tail = 0;
...
bool ESP32CAN::processFrame(twai_message_t &frame)
{
    ...
    for (int i = 0; i < BI_NUM_FILTERS; i++)
    {
        ...
        if ((msg.id & filters[i].mask) == filters[i].id && (filters[i].extended == msg.extended))
        {
            ...
            if (rxRing)
            {
                uint32_t next = (rxRing->head + 1) & RX_RING_MASK;

                if (next != rxRing->tail)
                {
                    rxRing->buffer[rxRing->head] = msg;
                    rxRing->head = next;
                }
                else
                {
                    rxOverflow = rxOverflow + 1;
                }
            }
...
#ifdef CONFIG_IDF_TARGET_ESP32S3
    return (rxRing.head - rxRing.tail) & RX_RING_MASK;
#else
    return (rxRing->head - rxRing->tail) & RX_RING_MASK;
#endif
}
...
#ifdef CONFIG_IDF_TARGET_ESP32S3
    if (rxRing.tail == rxRing.head)
        return false;
    msg = rxRing.buffer[rxRing.tail];
    rxRing.tail = (rxRing.tail + 1) & RX_RING_MASK;
    return true;
#else
    if (rxRing->tail == rxRing->head)
        return false;
...
uint32_t ESP32CAN::getOverflowCount()
{
    return rxOverflow;
}