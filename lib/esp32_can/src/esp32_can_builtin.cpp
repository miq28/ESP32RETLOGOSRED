...
uint32_t ESP32CAN::set_baudrate(uint32_t ul_baudrate)
{
    ...
    // Prevent rapid repeated baud changes (SavvyCAN sometimes sends them quickly)
...
{
    // stop RX processing immediately
    readyForTraffic = false;

    // allow RX task to exit twai_receive()
    ...
    if (task_CAN_handler)
    {
        ...
        task_CAN_handler = NULL;
    }

    if (task_LowLevelRX_handler)
    {
        vTaskDelete(task_LowLevelRX_handler);
        task_LowLevelRX_handler = NULL;
    }
